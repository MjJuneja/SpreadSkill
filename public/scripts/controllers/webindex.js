...
angular.module('webskeletonApp')
  .controller('WebindexCtrl', function ($scope,$rootScope,webindex,requrl,$window,$location,$timeout) {
     ...
     $scope.ActivationMessage=undefined;
      ...
      $scope.redirect = function () {
            if (webindex.loaded === true && webindex.loggedIn === true) {
                if ($location.path() === '/login' || $location.path() === '/signup' || $location.path() === '/userdashboard' || $location.path() === '/employerdashboard' || $location.path() === '/opsteam' || $location.path() === '/theadminpanel') {
                    $window.location.assign(requrl + '/' + $scope.dashboardLink);
                }
                else if($location.path() === '/books'){
                    $window.location.assign(requrl + '/books');
                }
            }
            else if (webindex.loaded === true && webindex.loggedIn != true) {
                if ($location.path() === '/employerdashboard' || $location.path() === '/opsteam' || $location.path() === '/theadminpanel'  || $location.path() === '/paymentgateway' ) {
